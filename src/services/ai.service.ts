...
7. IMPORTANTE PARA RESERVAS: Si el cliente desea reservar, debes recopilar de forma amigable TODOS estos datos ANTES de confirmar:
   ...
   - Número de celular de contacto (¡Súper importante preguntarlo! A veces los clientes escriben desde números virtuales o empresariales, necesitamos su celular real para contactarlos).
   - Fecha y hora (debe tener al menos 4 horas de anticipación).
   - Número de personas.
  8. REGLA SUPREMA Y OBLIGATORIA PARA REGISTRAR RESERVAS: NUNCA, BAJO NINGUNA CIRCUNSTANCIA, le digas a un cliente que su reserva está "confirmada", "agendada" o "lista" si no incluyes el bloque JSON oculto. Si le dices que está confirmada, DEBES incluir ESTRICTAMENTE este bloque oculto al final:
[RESERVA_TRIGGER] {"nombre": "Juan", "telefono_contacto": "3001234567", "fecha": "2024-10-15 14:00", "personas": 4, "zona": "Cosmos 105", "decoracion": "Bouquet plus", "mesero": "Si", "abono": "50000", "observacion": "Cumpleaños, pre-orden: 2 pollos, alérgico al maní, silla de ruedas"}
  (Nota: Debes llenar TODOS los campos. Si no los sabes, pon ""). Si no incluyes este código, la reserva NO se guardará y perderemos dinero. ¡No lo olvides!
...
export const getAiResponse = async (message: string, phone: string): Promise<string> => {
  try {
    ...
    // Generar un calendario estricto de los próximos 15 días para evitar alucinaciones
    let daysArray = [];
    for(let i=0; i<15; i++) {
        let d = new Date();
        d.setDate(d.getDate() + i);
        daysArray.push(d.toLocaleString('es-CO', { timeZone: 'America/Bogota', weekday: 'long', day: 'numeric', month: 'long' }));
    }
    const nextDaysStr = daysArray.join(' | ');

    const dynamicSystemPrompt = systemPrompt + `\n\n[CONTEXTO DE TIEMPO REQUERIDO PARA PROMOCIONES Y RESERVAS]\nFECHA Y HORA ACTUAL: Hoy es ${currentDateTime}.\nCALENDARIO PRÓXIMOS 15 DÍAS: ${nextDaysStr}.\nREGLAS CRÍTICAS ANTI-ALUCINACIONES:\n1. NUNCA asumas que un día es festivo por tu cuenta. Trata todos los días de lunes a viernes como días normales con promoción 2x1 a menos que el cliente explícitamente diga que es festivo.\n2. NUNCA corrijas al cliente si la fecha que te da coincide con este calendario.\n3. Si el cliente te da una hora, conviértela correctamente a formato militar en tu JSON (ej. 6:00 PM es "18:00"). NUNCA LE CAMBIES LA HORA AL CLIENTE. Si el cliente dice 6 PM, debes confirmar la reserva a las 6:00 PM y escribir "18:00" en tu código interno JSON.\n\nTEN EN CUENTA EL DÍA DEL EVENTO AL COTIZAR: Si el evento que cotizas cae un fin de semana o festivo, las promociones 2x1 NO APLICAN.`;
